def getoutput(result, grid_width, tile_count):
    ...
    tile_width = 4
    grid_output = seperateOutput(output_variable_value_maps, int(grid_width / tile_width))
...
def seperateOutput(original_list, size):
    seperated_list = []
    for i in range(0, len(original_list), size):
        seperated_list.append(original_list[i:i + size])
